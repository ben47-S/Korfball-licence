 ...
 *
 * @body InscriptionLicenceInput - Données validées par Zod
...
export async function POST(req: Request) {
  try {
    ...
    const validatedData = inscriptionLicenceSchema.parse(body);

    // 4. Créer l'inscription via le service
...
/**
 * GET /api/inscriptions?numeroLicence=xxx&dateNaissance=xxx&telephone=xxx&forRenewal=true
 ...
 * - Vérification que les informations correspondent au joueur
...
export async function GET(req: Request) {
  try {
    ...
    const licence = await InscriptionService.getLicenceByCredentials(
      numeroLicence,
      dateNaissance,
      telephone,
